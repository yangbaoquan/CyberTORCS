...
static void userDriverGetParam(float midline[200][2], float yaw, float yawrate,
                               float speed, float acc, float width, int gearbox,
                               float rpm);
static void userDriverSetParam(float *cmdAcc, float *cmdBrake, float *cmdSteer,
                               int *cmdGear);
...
extern "C" int driver_cruise(tModInfo *modInfo) {
  ...
  modInfo[0].desc =
      "user module for CyberCruise"; // Description of the module (can be long).
...
static void userDriverSetParam(float *cmdAcc, float *cmdBrake, float *cmdSteer,
  ...
  if (parameterSet == false) // Initialization Part
  {
    ...
    /*
            ...
            Enjoy  -_-
            */
    startPoint = int(_speed * 0.35);
    c = getR(_midline[startPoint][0], _midline[startPoint][1],
             _midline[startPoint + delta][0], _midline[startPoint + delta][1],
             _midline[startPoint + 2 * delta][0],
             _midline[startPoint + 2 * delta][1]);
    if (c.r <= 50) {
      expectedSpeed =
          constrain(45, 100, c.r * c.r * (-0.046) + c.r * 5.3 - 59.66);
    } else {
    ...
    if (curSpeedErr > 0) {
      if (abs(*cmdSteer) < 0.6) {
        *cmdAcc = constrain(0.0, 1.0,
                            kp_s * curSpeedErr + ki_s * speedErrSum + offset);
        *cmdBrake = 0;
     ...
     * Wei********************************************/
    /*
            Please select a error model and coding for it here, you can modify
       the steps to get a new 'D_err',this is just a sample. Once you choose the
       error model, you can rectify the value of PID to improve your control
       performance. Enjoy  -_-
            */
    // Direction Control
    ...
    D_err = -atan2(
        _midline[5][0],
        _midline[5][1]); // only track the aiming point on the middle line
    ...
    //        *cmdSteer = constrain(-1.0, 1.0, kp_d * D_err + ki_d * D_errSum +
    //        kd_d * D_errDiff + 0.06 * _yaw);
    *cmdSteer = constrain(-2.0, 2.0, kp_d * D_err + 11 / c.r + 2 * _yaw);
